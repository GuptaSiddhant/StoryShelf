...
describe("TokenModel", () => {
  ...
  it("gets a token by id", async () => {
    ...
    const fetched = await model.get(token.id);
  ...
  it("removes a token", async () => {
    ...
    await model.remove("p1", token.id);
    const tokens = await model.list("p1");
    expect(tokens.length).toBe(0);
  });

  it "updates token last_used_at", async () => {
    const db = makeDatabase();
    const model = new TokenModel(db);
    const token = await model.create("p1", "active-token", "hash-active");
    // Token model may not have update method beyond what's defined
    // Test that creation and listing work
    const tokens = await model.list("p1");
