...
export function resolveLabelLink(template: string | null, project: { gitRepository: string | null }, build: { gitBranch: string }, value: string): string | null {
  if (!template) {
    return null;
  }
  const repo = project.gitRepository ?? "";
  return template.replaceAll("{value}", value).replaceAll("{repo}", repo).replaceAll("{branch}", build.gitBranch);
}

/** Label page: every build bearing `key=value`, latest first (ADR 0013). */
export async function renderLabelDetailPage(slug: string, key: string, value: string): Promise<RenderedContent | null> {
  ...
  return (
    <DocumentLayout title={`${key}: ${value}`} nav={{ active: "labels", projectSlug: project.slug, projectName: project.name }}>
        ...
        <div class="page-header__row">
          <div>
            ...
            <p class="page-header__desc">
              <Badge tone="neutral">{key}</Badge> Builds bearing this {labelType ? labelType.name.toLowerCase() : "label"}, latest first.
            </p>
          </div>
      ...
      {builds.length === 0 ? (
        <div class="empty">
          ...
          <p class="empty__desc">No build currently bears the label {key}: {value}.</p>
        </div>
      ) : (
            ...
            <tbody>
              {builds.map(
                (build): HtmlEscapedString | Promise<HtmlEscapedString> => {
                  const link = labelType?.linkTemplate ? resolveLabelLink(labelType.linkTemplate, project, build, value) : null;
                  return (
                    <tr key={build.id}>
                      <td>
...
export async function renderLabelsPage(slug: string): Promise<RenderedContent | null> {
  ...
  return (
    <DocumentLayout title={`Labels · ${project.name}`} nav={{ active: "labels", projectSlug: project.slug, projectName: project.name }}>
      <div class="page-header">
        <nav class="breadcrumbs" aria-label="Breadcrumb">
          <ol>
            <li>
        ...
        <div class="page-header__row">
          <div>
            ...
            <p class="page-header__desc">Typed labels attach values to builds and link out to external systems.</p>
          </div>
        </div>
      </div>
