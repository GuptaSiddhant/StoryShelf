    ...
    <DocumentLayout title="Projects" nav={{ active: "projects" }}>
      <div class="page-header">
        <div class="page-header__row">
          <div>
            ...
            <p class="page-header__desc">Each project is one Storybook. Create a project, then upload builds from CI.</p>
          </div>
      ...
      {projects.length === 0 ? (
        <div class="empty">
          ...
          <p class="empty__desc">Create your first project to start visual testing. Projects are free and unlimited.</p>
          {canCreate ? (
                ...
                <div style="display:flex; justify-content:space-between; gap:1rem; flex-wrap:wrap; align-items:flex-start;">
                  <div style="min-width:0;">
                    ...
                    <p class="field__hint" style="margin:.2rem 0 0;">
                      <code>{project.slug}</code> {project.gitRepository ? `· ${project.gitRepository}` : ""} · default <Badge tone="neutral">{project.gitDefaultBranch}</Badge>
                    </p>
                    {info?.latest ? (
