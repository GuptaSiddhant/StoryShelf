    ...
    <div class="grid" style="max-width: 720px;">
      ...
      <div class="card card--padded">
        ...
        <p class="field__hint" style="margin-bottom:1rem;">
          When enabled, Storybook <code>play</code> functions run before each screenshot. Failures block the build unless the story is marked
          <code>flaky-test</code> via <code>tags: ['flaky-test']</code> or
          <code>parameters: &#123; flakyTest: true &#125;</code> (supports both <code>chromatic</code> and <code>storyshelf</code> keys, story-level, case-insensitive). Use{" "}
          <code>disableSnapshot</code> to skip a story entirely.
        </p>
        <form
          method="post"
        ...
        >
          <div class="field">
            <label class="field__label" style="display:flex; gap:.5rem; align-items:center;">
              <input type="checkbox" name="executePlay" value="true" checked={project.executePlay ? true : undefined} disabled={!isAdmin} />
              Enable interaction tests (play)
            </label>
            <p class="field__hint">When enabled, failing play blocks the build (failed). Flaky stories show warnings but status stays successful.</p>
          </div>
          <div class="field">
            ...
            <input
              ...
              disabled={!isAdmin}
            />
            <p class="field__hint">Timeout for each play function (1000–30000 ms, default 10000).</p>
          </div>
          {isAdmin ? (
      ...
      <div class="card card--padded">
        ...
        <pre class="field__hint" style="white-space:pre-wrap; background: var(--surface); padding:.75rem; border-radius:.5rem;">
...
export const Flaky2: Story = {
  parameters: { chromatic: { flakyTest: true } }
        ...
        <p class="field__hint">
          Tags are story-level and case-insensitive (<code>flaky-test</code>). Whole story is non-blocking when flaky. GitHub status stays
