  ...
  return (
    <div class="grid" style="max-width: 880px;">
      ...
      <div class="card card--padded">
        ...
        <p class="field__hint">
          Notify external services when builds are created, updated, approved or rejected. Payloads are POSTed as JSON and signed with the webhook secret.
        </p>
              ...
              {webhooks.map(
                (webhook): HtmlEscapedString | Promise<HtmlEscapedString> => (
                  <tr key={webhook.id}>
                    ...
                    <td>
                      {isAdmin ? (
                        <form method="post" action={`/projects/${project.slug}/settings/webhooks/${webhook.id}/delete`} hx-post={`/projects/${project.slug}/settings/webhooks/${webhook.id}/delete`} hx-target="body">
        ...
        {webhooks.length === 0 ? <p class="field__hint" style="margin-top:.5rem;">No webhooks configured.</p> : null}
      </div>

      {isAdmin ? (
        <div class="card card--padded">
          <h3 style="margin:0 0 .5rem;">Create webhook</h3>
          <form method="post" action={`/projects/${project.slug}/settings/webhooks`} hx-post={`/projects/${project.slug}/settings/webhooks`} hx-target="body">
            <div class="field">
              <label class="field__label" for="url">
                URL
              </label>
              <input class={`field__input ${formState?.errors?.["url"] ? "field__input--error" : ""}`} id="url" name="url" type="url" required placeholder="https://example.com/hooks/storyshelf" />
            ...
            <div class="field">
              ...
              <input class="field__input" id="events" name="events" placeholder="build.created, build.approved, snapshot.reviewed" />
