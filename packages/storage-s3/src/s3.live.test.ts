/**
 * Live S3 storage test — strictly real cloud, no fakes or emulators.
 *
 * Gated on `LIVE_CLOUD=1` so hermetic `turbo test` never touches AWS.
 * Bucket is pre-provisioned (manually or via `terraform apply` from
 * `storyshelf server init --target aws`); this suite only owns keys under
 * a unique per-run prefix and deletes them afterwards.
 *
 * Required env when live:
 * - `S3_BUCKET` — existing bucket (from `terraform output -json` → `s3_bucket`)
...
function liveBucket(): string {
  ...
  if (!bucket) {
    throw new Error("Live S3 test requires S3_BUCKET (terraform output: s3_bucket).");
  }
  return bucket;
}
...
describe.skipIf(!LIVE)("s3 live (real AWS, gated on LIVE_CLOUD=1)", () => {
  const prefix = runPrefix();
  let storage: StorageAdapter;

  function activeStorage(): StorageAdapter {
    if (!storage) {
      throw new Error("Live S3 harness not initialized");
    }
    return storage;
  }

  beforeAll(async () => {
    storage = createS3Storage({ bucket: liveBucket(), prefix, region: liveRegion() });
  ...
  it("lists only keys under the run prefix", async () => {
    const target = activeStorage();
