...
// Keep each package's JSR config `version` in sync with its package.json version.
// The config is regenerated by tsdown's `onSuccess` during builds; here we apply
// the bumped version to the committed files so the tree stays consistent between
// a `nub version` bump and the publish-time build in CI. Prefers `deno.json`
// when present (core: carries the JSX import map), else `jsr.json`.
function syncJsrVersions(packages) {
  const packagesDir = new URL("../packages/", import.meta.url).pathname;
  for (const { dir, version } of packages) {
    for (const file of ["deno.json", "jsr.json"]) {
      const jsrPath = join(packagesDir, dir, file);
      try {
        ...
        // Missing or invalid config — try the next filename
      }
    }
  }
}
...
// 5.5 Sync jsr.json versions to the bumped package version (tsdown regenerates on build too).
syncJsrVersions(after);
