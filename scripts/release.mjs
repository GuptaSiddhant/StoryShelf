...
// Keep each package's jsr.json `version` in sync with its package.json version.
// jsr.json is regenerated by tsdown's `onSuccess` during builds; here we apply the
// bumped version to the committed files so the tree stays consistent between a
// `nub version` bump and the publish-time build in CI.
function syncJsrVersions(packages) {
  ...
  for (const { dir, version } of packages) {
    ...
    try {
      ...
      // Missing or invalid jsr.json — nothing to sync
    }
  }
}
